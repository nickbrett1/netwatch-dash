...
from . import buildinfo, snapshot
...
def healthz_body(settings: Settings, now: datetime | None = None) -> dict:
    """The whole of ``/healthz``, as a plain function so it is testable alone."""
    snap = snapshot.build(settings, now)
    ...
    return {
        ...
        # number rather than a silently missing panel.
        "data": {"drift_count": drift.count, "drift": drift.as_dict()},
...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    def api_summary() -> dict:
        ...
        return snapshot.build(app.state.settings).summary()

    @app.get("/api/probe")
    def api_probe(limit: int = Query(PROBE_DEFAULT_LIMIT, ge=1, le=1000)) -> dict:
        return snapshot.build(app.state.settings).collection("probe", limit)

    @app.get("/api/speed")
    def api_speed(limit: int = Query(SPEED_DEFAULT_LIMIT, ge=1, le=1000)) -> dict:
        return snapshot.build(app.state.settings).collection("speed", limit)
