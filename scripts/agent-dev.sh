...
# "netwatch-dash-dev" (the repo name plus the "-dev" suffix).
...
# than adding a second one - a stale "netwatch-dash-dev" is overwritten by
...
PROJECT_NAME="netwatch-dash"
AGENT_NAME="netwatch-dash-dev"
WORKSPACE="/workspaces/netwatch-dash"
...
if [ -x "/workspaces/netwatch-dash/scripts/agent-dev.sh" ]; then
  "/workspaces/netwatch-dash/scripts/agent-dev.sh" start >/dev/null 2>&1 || true
fi
POST_START_HOOK
}
