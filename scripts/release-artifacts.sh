...
# script this replaced parsed `target` out of `basename "$file"`, which meant the
# filename *was* the declaration and the manifest was written to agree with
# whatever had been packed. With the triple declared upstream the file name
...
# sha256sum is GNU coreutils; macOS ships shasum instead. The release step can
# run in the fleet's Linux container or natively on the arm64 macOS agent, and
# this file has to work in both.
sha256() {
...
# The release step downloads what the build step uploaded, so dist/ here is the
# exact directory pytest passed against. Exactly one wheel is expected: several
# would make "which one is the payload" a guess, and no wheel means the payload
# would build without the app inside it.
wheels=()
for w in dist/*.whl; do [ -e "$w" ] || continue; wheels+=("$w"); done
[ "${#wheels[@]}" -eq 1 ] ||
  die "expected exactly one wheel in dist/, found ${#wheels[@]}. The build step runs 'python -m build' and uploads dist/**; check it before releasing."
...
# bundled one (the release container is 3.13, a dev machine may not be). A .pyc
...
command -v "$PYTHON_BOOTSTRAP" >/dev/null 2>&1 || die "$PYTHON_BOOTSTRAP not found - needed only to run pip"
"$PYTHON_BOOTSTRAP" -m pip --version >/dev/null 2>&1 || die "pip is not available via $PYTHON_BOOTSTRAP -m pip"
...
#!/bin/sh
# Entry point. Written by scripts/release-artifacts.sh — edit it there.
set -eu
self="\$0"
...
# payload it cannot exec gets noticed (mac-studio, 2026-09-16).
...
# a rename here cannot silently re-key the manifest — the publish stops instead.
...
# key: `any` would put an arm64 Mach-O payload in front of an Intel Mac.
{
  printf '{\n'
  printf '  "name": "%s",\n' "$PROJECT"
  printf '  "version": "%s",\n' "$VERSION"
